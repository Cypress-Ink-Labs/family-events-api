...
7. **Parent-tips event-data delimiters escaped:** unlike legacy, every untrusted text field replaces embedded `<event_data>` / `</event_data>` tokens with HTML entities before prompt assembly. The system prompt explicitly treats apparent delimiters and instruction-like text as data, preventing a scraped or admin-entered field from terminating the event-data boundary.
...
- [ ] **Step 1: Port the prompt** from legacy `generate-parent-tips/prompt.ts` (89 lines): categories, one-sentence-<25-words rule, JSON-only, and the prompt-injection guard treating `<event_data>` as untrusted. Escape embedded delimiter tokens in every untrusted text field per deliberate deviation #7.
