...
// mapped (missing row, duplicate, bad enum text) land here instead. Bodies
// use the same { statusCode, message, error } envelope Nest builds for
// HttpExceptions so clients need no special-casing for PG-mapped errors.
const PG_ERROR_RESPONSE: Record<string, MappedStatus> = {
  ...
  "23505": { status: HttpStatus.CONFLICT, message: "record already exists", error: "Conflict" },
  "23502": {
    status: HttpStatus.BAD_REQUEST,
    message: "missing required value",
    error: "Bad Request",
  },
  "23514": { status: HttpStatus.BAD_REQUEST, message: "invalid value", error: "Bad Request" },
  "22P02": { status: HttpStatus.BAD_REQUEST, message: "invalid identifier", error: "Bad Request" },
}
