...
async function seedCity(
  ...
  await db.query(
    `INSERT INTO public.cities (id, name, state, slug, timezone, latitude, longitude)
     VALUES ($1, 'Lafayette', 'LA', $2, 'America/Chicago', $3, $4)`,
...
async function eventById(id: string) {
  const rows = await db.query<{
    ...
    `SELECT latitude::text, longitude::text, images, last_enrichment_attempt_at,
            parent_tips, parent_tips_provider, parent_tips_model, parent_tips_prompt_version
