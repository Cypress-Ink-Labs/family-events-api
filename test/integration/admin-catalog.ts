...
export async function ensureAdminCatalog(db: DbService): Promise<void> {
  ...
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS private.railway_cron_runs (
      ...
      run_key uuid NOT NULL DEFAULT gen_random_uuid() UNIQUE,
      label text NOT NULL,
    ...
    ALTER TABLE private.cron_enabled
      ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
    ALTER TABLE private.railway_cron_runs
      ADD COLUMN IF NOT EXISTS run_key uuid DEFAULT gen_random_uuid();
    UPDATE private.railway_cron_runs SET run_key = gen_random_uuid() WHERE run_key IS NULL;
    ALTER TABLE private.railway_cron_runs
      ALTER COLUMN run_key SET DEFAULT gen_random_uuid(),
...
export async function truncateAdminCatalog(db: DbService): Promise<void> {
  ...
  await db.query(
    "TRUNCATE private.cron_run_log_entries, private.railway_cron_runs, private.cron_enabled, private.test_email_dispatch_failure, public.invite_requests, public.invite_codes, public.user_access, auth.users CASCADE"
  )
}
